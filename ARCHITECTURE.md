...
- Update (2025-12-01): Implemented chat transcript persistence for issue #37. Transcripts are stored as JSON files in `%APPDATA%\JuniorDev\ChatTranscripts\` with configurable pruning by message count, size, and age. User messages saved on send, AI responses captured via periodic polling of DevExpress AIChatControl. Load on attach/create with configurable context message limit. Corruption handling falls back to empty transcript.
